...
parser.add_argument('--outf', default='/home/user_1/zhangwenzhen/resnet-Clothing1M/model',
                    help='folder to output images and model checkpoints')  # 输出结果保存路径
parser.add_argument('--net', default='/home/user_1/zhangwenzhen/resnet-Clothing1M/model/Resnet18.pth',
                    help="path to net (to continue training)")  # 恢复训练时的模型路径
parser.add_argument('--data_path', default='/home/user_1/zhangwenzhen/clothing1M/images', type=str, help='path to dataset')
...
parser.add_argument('--early_stop', default=1, type=int, help='early stop epoch')
parser.add_argument('--dataset', default='Clothing1M_lr0.01_0.1_L1_0.05', type=str)
...
parser.add_argument('--num_t', default=2000*64, type=int)
...
parser.add_argument('--L1_rate', default=0.5, type=float, help='sp in sharp rate')
parser.add_argument('--logs_path', default='classification_L1/',
...
#=========clothing1M_data  loader===================================================================================
loader = clothing_dataloader(root=args.data_path, batch_size=args.batch_size,
                                        num_workers=8,
...
r_2_1 = 0.0
# ==============================================
y_c = np.zeros([args.num_t, clas])
y_nc = np.zeros([args.num_t, clas])
epoch_step = [10,20]
label_red3 = torch.zeros([args.num_t, clas], dtype=torch.float).to(device)
label_red3t = torch.zeros([args.num_t, clas], dtype=torch.float).to(device)
num_examp = args.num_t

label_mix_hist = torch.zeros([num_examp, clas], dtype=torch.float).to(device)
label_rates = torch.ones([args.num_t], dtype=torch.float).to(device)
label_up = torch.ones([args.num_t], dtype=torch.float).to(device)
...
experiment_name = "{}_batch_{}_stop_{}_sp_{}_seed_{}_alpha_{}_boost_{}_lr_{}_{}".format(args.dataset, args.batch_size, args.early_stop,
                                                           args.L1_rate, args.seed, args.alpha, args.boost, args.lr, args.tosharp)
            ...
            for epoch in range(pre_epoch, EPOCH):
                ...
                for i, (input, label, index) in enumerate(trainloader):
                    ...
                    if epoch1 <= args.early_stop:
                        label_red3[index_t] = F.one_hot(labels_two, num_classes=clas).detach() * 1.000

                    else:
                        pred_detach_max, pred_detach_max_dim = pred_detach.max(dim=1)
                    ...
                    feature_relation = torch.torch.matmul(pred_s_t, pred_s)
                    L1_feature = torch.norm(feature_relation * (torch.ones(clas, clas) - torch.eye(clas)).to(device), p=1)
                    print("L1_feature", L1_feature.detach())
                    # ======================================================================
                    # loss1 = -((label_red3[index_t] * torch.log_softmax(outputs, dim=1)).sum(dim=1)).mean() + r_2_1*L1_feature# * p1).sum() / p1.sum()#(criterion_1(outputs, labels_two)).mean()  # r3_max / r3_max.mean() *  #criterion(outputs, labels_two) + 7.0*elr_reg1#
                    if epoch1 <= 1:
                        loss1 = -((label_red3[index_t] * torch.log_softmax(outputs, dim=1)).sum(dim=1)).mean()
                    else:
                        loss1 = -((label_red3[index_t] * torch.log_softmax(outputs, dim=1)).sum(
                            dim=1)).mean() + 0.03 * L1_feature  # loss_mix
                    ...
                    if epoch1 == 1:
                        # l11 += loss_mix.detach()  #
                        l11 += (-((label_red3[index_t] * torch.log_softmax(outputs, dim=1)).sum(dim=1)).mean()).detach()
                        l22 += L1_feature.detach()
                    print('r_2_1r_2_1r_2_1r_2_1r_2_1r_2_1r_2_1r_2_1r_2_1r_2_1r_2_1r_2_1', r_2_1)
                if epoch1==1:
                    l11 /= (i+1)
                    l22 /= (i+1)
                    r_2_1 = l11 * args.L1_rate / l22
                #===============================================================================================
                lp = sum_loss / (i + 1) * ltr
                ltr *= 1.001
                print('lp: ', lp)
