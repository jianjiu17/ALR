...
parser.add_argument('--r', default=0.2, type=float, help='noise_ratio')
...
parser.add_argument('--dataset', default='cifar100_123_yessharpe', type=str)
parser.add_argument('--num_epoch', default=300, type=int, help='epoch')
...
parser.add_argument('--logs_path', default='classification_logs_300_ablation/',
...
class HardBootstrappingLoss1(nn.Module):
    ...
    def forward(self, y_pred, y, y_cs, y_val, epoch_num=0):
        ...
        if epoch_num > 2000:   #20
            ...
            tk = (s * (max_1/max_2 > max_2/max_10)*(fy< max_3)*(max1_mun==y_cs) + (torch.ones_like(s) - s) * (max1_mun ==y)*(max1_mun==y_cs)).sum()#((max1_mun == y_cs) * (max_1 / max_2 > max_2 / max_3) * (max_1 / max_2 > max_2 / max_10)).sum()
            tk2 = (s * (max_1/max_2 > max_2/max_10)*(fy< max_3) + (torch.ones_like(s) - s) * (max1_mun ==y)).sum() #((max_1 / max_2 > max_2 / max_3) * (max_1 / max_2 > max_2 / max_10)).sum()
            ...
            print('fy===last4-7::', ((fy >= max_7) *(fy <= max_4) * s * (max1_mun == y_cs)).sum())  # 认定的噪声中预测是对的的样本的噪声标签小于max_7  ,15%

            bootstrap = F.cross_entropy(y_pred, z1, reduction='none') * max_1.detach()#* torch.from_numpy(ycc_bh*2.0 + (1.0-ycc_bh)*0.5).to(device) #(torch.ones_like(s)-(s*(max_1 <= k_7)))#* max_1.detach()   #* (s*0.4 + (torch.ones_like(s) - s) *fy.detach())
            ...
            tk = ((max1_mun == y_cs) * (s * (max_1 / max_2 > max_2 / max_10) * (fy < max_3)+(torch.ones_like(s) - s) * (max1_mun == y))).sum()
            tk2 = (s * (max_1 / max_2 > max_2 / max_10) * (fy < max_3)+(torch.ones_like(s) - s) * (max1_mun == y)).sum() #((max_1 / max_2 > max_2 / max_3) * (max_1 / max_2 > max_2 / max_10)).sum()
            print('fy===last::', ((fy < max_7) * s * (max1_mun == y_cs)).sum())

            bootstrap = F.cross_entropy(y_pred, z1, reduction='none') * y_local #fy.detach()   #* (torch.ones_like(s)-(s*(max_1 <= k_7)))             #* torch.pow(fy.detach(), 1)

        if self.reduce:

            zsmall = s * max1_mun * (max_1/max_2 > max_2/max_10)*(fy< max_3) + ((torch.ones_like(s)-s) * (max_1 ==y))* y +(torch.ones_like(s)-(s * (max_1/max_2 > max_2/max_10)*(fy< max_3) + (torch.ones_like(s)-s) * (max_1 ==y))) * z1_s #三部分，1是选择的噪声中的可靠的，2是正确中的可靠的，3是其余的做均匀噪声
            zao1 = (torch.where(s * (max_1 / max_2 > max_2 / max_10) * (fy < max_3) ==1, max1_mun.to(device), torch.ones_like(s).long() * (-1)) == y_cs).sum()
            clean1 = (torch.where(((torch.ones_like(s)-s) * (max_1 ==y))==1, y, torch.ones_like(s).long() * (-1))==y_cs).sum()
            last1 = (torch.where((torch.ones_like(s)-(s * (max_1/max_2 > max_2/max_10)*(fy< max_3) + (torch.ones_like(s)-s) * (max_1 ==y)))==1, z1_s, torch.ones_like(s).long() * (-1))==y_cs).sum()
            print('z1  clean raze', zao1, clean1, last1, (zsmall == y_cs).sum(), (z1 == y_cs).sum())
            print('z1', z1)

            return torch.mean(bootstrap), fy_cs_num, max_1, max_2, max_3, max_4, max_5, max1_mun, tk, tk2, z1, z1, s  # zsmall, z1       bootstrap
...
def multiclass_noisify(y, P, random_state=0):
    ...
    for idx in np.arange(m):
        #i = y[idx]
        i = int(y[idx])     # there is problem of decimals in imported yy,which is also the modified label, int transpose is adopted here!!!
...
optimizer = optim.SGD(net.parameters(), lr=0.02, momentum=0.9, weight_decay=1e-3) #优化方式为mini-batch momentum-SGD，并采用L2正则化（权重衰减）
...
#=====================================================================================================
experiment_name = "{}_batch_{}_stop_{}_sp_{}_{}_{}_seed_{}_boost_{}_{}".format(args.dataset, args.batch_size, args.early_stop,
                                                           args.sharp_rate, args.noise_type, args.r, args.seed, args.boost, args.tosharp)
        ...
        with open('outputs_softmax.txt', 'w') as f4:
            for epoch in range(pre_epoch, EPOCH):
                ...
                for i, (input, label, label_c, index) in enumerate(trainloader):
                    ...
                    outputs_softmax, outputs_softmax_index = torch.topk(nn.functional.softmax(outputs, 1), k=2, dim=1)#.cpu().detach().numpy()
                    ...
                    r3_mix = label_red3_one * (r3_max >= r3_max.mean()).unsqueeze(1).repeat(1, clas).type(torch.float) + label_red3[index_t]*(r3_max < r3_max.mean()).unsqueeze(1).repeat(1, clas)
                    ...
                    L2_feature = torch.norm(feature_relation*(torch.ones(clas,clas) - torch.eye(clas)).to(device), p=1, dim=1)
                    L2_1_feature = torch.mean(L2_feature)

                    loss_r = -((label_red3[index_t] * torch.log_softmax(outputs, dim=1)).sum(dim=1)).mean()  #+ L2_1_feature #- (torch.log(1.0-pred_s*(pred_s<pred_min50.unsqueeze(1).repeat(1,clas).type(torch.float))).sum(dim=1)).mean()
                    ...
                    if epoch1<=args.boost:   # 40,0.3  ==70+
                        ...
                        loss1 = loss_r - args.sharp_rate*epoch_rate*((F.softmax(outputs, dim=1) * torch.log_softmax(outputs, dim=1)).sum(dim=1)).mean() #- 3.0*((F.softmax(outputs, dim=1) * torch.log_softmax(outputs, dim=1)).sum(dim=1)).mean()
                    ...
                    loss2, pre_noise, label_jiu, lab9, lab8, jiu_no, max911 = loss_HB(outputs, labels_two)  # pre_noise 是选中的噪声  labels_two = labels
                    ...
                    pre_max_sum += ((pre_max2 < rat_avg) * (pre_max_dim == label_cs)).sum()     #预测的max2小于0.1 且 预测值等于真实值的概率
                    pre_cs += (pre_max_dim == label_cs).sum()   #预测的==真正标签的概率
                    pre_lab_cs += ((pre_max_dim == label_cs) * (labels_two == label_cs)).sum()   #预测的==标记的==真正的标签  的概率
                    pre_max_max += (pred_y_pred>0.01).sum()#((pre_max2 < rat_avg)*(pre_max > 0.8)*(pre_max_dim == label_cs)).sum()      #预测的max2小于0.1的概率
                    ...
                    if epoch1 < 1000:
                        ...
                        y_num_count[index,:] = F.one_hot(tongji,num_classes=clas).data.cpu().numpy()            #        F.one_hot(max10_d, num_classes=10)
                    ...
                    print(
                        ...
                        % (epoch + 1, (i + 1 + epoch * length), sum_loss / (i + 1), 100. * train_correct / total,
                           ...
                           100. * jiu_num9 / total, 100. * jiu_num8 / total, 100. * pre_max_sum / total, 100. * pre_max_max / total))
                    f2.write(
                        ...
                        % (epoch + 1, (i + 1 + epoch * length), sum_loss / (i + 1), 100. * train_correct / total,
                           100. * (1 - noise_num / total), 100. * noise_jisuan / total,
                           100. * noise_zide / total, 100. * jiu_num / total, 100. * jiu_num10 / total,
                           100. * jiu_num9 / total, 100. * jiu_num8 / total, 100. * last4 / total, 100.0 * pre_max_sum / total, 100. * pre_max_max / total, 100. * pre_cs / total, 100. * pre_lab_cs / total, 1.00/r3_min.mean(), (1 - (label_red3[index_t] * pred_s).sum(dim=1)).mean(), 100. * dimt_noise / total))
                    f2.write('\n')
                    f2.flush()
