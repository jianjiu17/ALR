...
for iteration in range(7):
    if iteration <=4:
        noise_tp = 'sym'
        r_i = 0.0 + 0.2 * iteration
    else:
        noise_tp = 'asym'
        r_i = 0.2 + 0.2 * (iteration-5)
    """noise_tp = 'asym'
    r_i = 0.4"""
    ...
    parser.add_argument('--outf', default='/home/user_1/zhangwenzhen/resnet-18-cifar10-ce/model/',
                        help='folder to output images and model checkpoints')  # 输出结果保存路径
    parser.add_argument('--net', default='/home/user_1/zhangwenzhen/resnet-18-cifar10-ce/model/Resnet18.pth',
    ...
    parser.add_argument('--boost', default=300, type=int)
    parser.add_argument('--num_epochs', default=210, type=int, help='epoch')
    parser.add_argument('--early_stop', default=300, type=int, help='early stop epoch')
    parser.add_argument('--tosharp', default='nodecline', type=str)
    parser.add_argument('--dataset', default='cifar10_seed123_ce', type=str)
    ...
    parser.add_argument('--logs_path', default='classification_logs_ce_125/',
    ...
    trainset = CIFAR10RandomLabels(root='./data', train=True, download=True, transform=transform_train, num_classes=10,
                                   corrupt_prob=args.r, noise_t=args.noise_type)
    """sample_radio = 0.1
    num_sample = int(len(trainsets)*sample_radio)
    selectd_indices = random.sample(range(len(trainsets)), num_sample)
    trainset = torch.utils.data.Subset(trainsets, selectd_indices)"""
    # trainset = CIFAR10RandomLabels_pred(root='./data', train=True, download=True, transform=transform_train, num_classes=10)
    ...
    # 划分训练集和测试集
    trainset, valset = torch.utils.data.random_split(trainsets, [num_train - num_test, num_test])"""

    trainloader = torch.utils.data.DataLoader(trainset, batch_size=BATCH_SIZE, shuffle=True, num_workers=2, pin_memory=True)
    # trainset =  torch.utils.data.ConcatDataset([trainset1, trainset_p])
    '''trainloader = torch.utils.data.DataLoader(
        CIFAR10RandomLabels(root='./data', train=True, download=True, transform=transform_train, num_classes=10,
    ...
    '''for name, param in net.named_parameters():
        if (name.startswith('conv1')):
            ...
            dattt = param.data.numpy().reshape(9,48)
            print(np.abs(dattt))
            imwrite('./data_co_0.8.jpg', dattt)
        if (name.startswith('fc.weight')):
            print('name:',name, 'param: ', param.data)
            dattt = param.data.numpy().reshape(128,10)
            print(np.abs(dattt))
            imwrite('./data1_fc_0.8.jpg', dattt)'''
    print('jieshu')
    # ====================================================
    ...
    label_red3 = torch.ones([50000, 10], dtype=torch.float).to(device) * 0.1  # elr
            ...
            with open('outputs_softmax.txt', 'w') as f4:
                for epoch in range(pre_epoch, EPOCH):
                    ...
                    for i, (input, label, label_c, index) in enumerate(trainloader):
                        ...
                        outputs, train_features = net(inputs)
                        ...
                        label_mix_hist[index] = 0.5*label_mix_hist[index] + 0.5*mix_pred / (mix_pred).sum(dim=1,keepdim=True)
                        ...
                        index = index.numpy()
                        if i==0:
                            features_all = train_features
                            labels_all = label_cs
                            labels_to = labels
                        else:
                            features_all = torch.cat((features_all, train_features))
                            labels_all = torch.cat((labels_all, label_cs))
                            labels_to = torch.cat((labels_to, labels))
                        ...
                        # pred_s = torch.clamp(pred_s, 1e-2, 1.0 - 1e-2)
                        ...
                        """label_new = (F.one_hot(labels_two, num_classes=10) * 1.000 + r3_max * torch.clamp((10.0 * label_red3[index_t] - label_red3[index_t].sum(dim=1, keepdim=True)), min=0)).detach()
                        row_sums = label_new.sum(dim=1, keepdim=True)
                        # 对每一行进行归一化
                        norm_label = label_new / row_sums"""
                        ...
                        # loss1 = criterion(outputs, labels_two) #+ 3.0*elr_reg1
                        loss_r = (-(label_red3[index_t] * torch.log_softmax(outputs, dim=1)).sum(dim=1)).mean()
                        # loss_r = (-(norm_label * torch.log_softmax(outputs, dim=1)).sum(dim=1)).mean()
                        if epoch1 <= args.boost:
                            loss1 = loss_r  # criterion(outputs, labels_two)
                            ...
                            # loss1 = (-(label_red3[index_t] * torch.log_softmax(outputs, dim=1)).sum(dim=1)*r3_max).sum() / r3_max.sum()
                        ...
                        '''if epoch1 <51:
                            ...
                            print('f-3max: ', F.softmax(outputs.detach(), dim=1).kthvalue(8, dim=1))
                            print('f-2max: ', F.softmax(outputs.detach(), dim=1).kthvalue(9, dim=1))
                            print('f-1max: ', F.softmax(outputs.detach(), dim=1).kthvalue(10, dim=1))
                            print('zzz', F.softmax(outputs.detach(), dim=1).gather(1, t).view(-1) < 0.1)
                            s = (F.softmax(outputs.detach(), dim=1).gather(1, t).view(-1) < 0.1).type(torch.int)
                            print('s', s, s.sum())
                    ...
                    labels_to = labels_to.cpu().numpy()


                    """if epoch1 ==199:
                        np.save(f'wusharpe_train_300_features_noise_{args.noise_type}_{args.r}_123.npy', features_all)
                        np.save(f'wusharpe_train_true_300_label_{args.noise_type}_{args.r}_123.npy', labels_all)
                        np.save(f'wusharpe_train_noise_300_label_{args.noise_type}_{args.r}_123.npy', labels_to)"""
                    if epoch1 < 1000:
                        # save y_tilde
                        y = new_y
                    ...
                    with torch.no_grad():
                        ...
                        for j, (data) in enumerate(testloader):
                            ...
                            test_outputs, test_features = net(images_test)  # the last one
                            """if j==0:
                                features_all = test_features
                                labels_all = labelts
                            else:
                                features_all = torch.cat((features_all, test_features))
                                labels_all = torch.cat((labels_all, labelts))"""
                        ...
                        if epoch1 ==199:
                            np.save(f'wusharpe_test_features_noise_{args.noise_type}_{args.r}_124.npy', features_all)
                            np.save(f'wusharpe_test_label_{args.noise_type}_{args.r}_124.npy', labels_all) """
