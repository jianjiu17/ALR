...
parser.add_argument('--early_stop', default=0, type=int, help='early stop epoch')
parser.add_argument('--dataset', default='webvision', type=str)
parser.add_argument('--model', default='InceptionResNetV2', type=str)
parser.add_argument('--tosharp', default='nodecline_elr_*0.1', type=str)
...
parser.add_argument('--lr', default=0.02, type=float, help='lr_sgd')
parser.add_argument('--sharp_rate', default=0.2, type=float, help='sp in sharp rate')
parser.add_argument('--logs_path', default='classification_logs_lr0.2_0.1/',
...
def adjust_learning_rate(optimizer, epoch):  # 带有warm-up的余弦学习率衰减
    ...
    for param_group in optimizer.param_groups:
        param_group['lr'] = lr
#=========================accuracy============================================================
def accuracy(outputs, target, topk=(1,5)):
    """Computes the precision@k for the specified values of k"""
    maxk = max(topk)
    output= F.softmax(outputs, dim=1)
    fifth_largest = output.topk(maxk, dim=1, largest=True, sorted=True)[0][:, -1]
    # 检查标签是否大于等于每个样本的第5大值
    correct = (output.gather(1, target.view(-1, 1)).squeeze() >= fifth_largest)
    # 计算正确的数量
...
# seed_torch(123)
epoch_save = -1
y_file = '/home/user_1/zhangwenzhen/resnet-Webvision/newlab2/' + "y.npy"
y_c_file = '/home/user_1/zhangwenzhen/resnet-Webvision/labcount1/' + "y_121.npy"  # 存储每轮预测为各标签的次数
# ======================================================
train_loss_n = 0.0
train_acc_n = 0.0
test_loss_n = 0.0
...
# =====================================================================================================
experiment_name = "{}_batch_{}_stop_{}_sp_{}_seed_{}_alpha_{}_boost_{}_lr_{}_{}_lr_{}".format(args.dataset, args.batch_size, args.early_stop,
                                                           args.sharp_rate, args.seed, args.alpha, args.boost, args.lr, args.tosharp, args.lr)
        ...
        with open('outputs_softmax.txt', 'w') as f4:
            for epoch in range(pre_epoch, EPOCH):
                ...
                if epoch1 in epoch_step:
                    ...
                    optimizer = optim.SGD(net.parameters(), lr * 0.1, momentum=0.9, weight_decay=5e-4)  # lr=0.3
                ...
                for i, (input, label, index) in enumerate(trainloader):
                    ...
                    if epoch1 <= args.boost:  # 40,0.3  ==70+
                        loss1 = criterion(outputs, labels_two) + 3*elr_reg1
                ...
                with torch.no_grad():
                    ...
                    for j, (data) in enumerate(testloader):
                        ...
                        pred_top5 = accuracy(test_outputs, labelts)
                        test_5_epoch += pred_top5
                        test_loss_n = (test_loss / (j+1)).data.cpu().numpy()
                        print("test is down")
                    #===========================================================================================
                    for j, (data) in enumerate(imagenet_valloader):
                        net.eval()
                        # net_class.eval()
                        images_test_2, labelts_2 = data
                        images_test_2, labelts_2 = images_test_2.to(device), labelts_2.to(device)
                        test_outputs_2, test_x_2, test_conf_2 = net(images_test_2)  # the last one

                        # outputs = net_class(output)
                        # 取得分最高的那个类 (outputs.data的索引号)
                        _, test_predicted_2 = torch.max(test_outputs_2.data, 1)


                        test_total_2 += labelts_2.size(0)
                        test_correct_2 += (test_predicted_2 == labelts_2).sum()
                        print("test_correct_2", test_correct_2)
                        pred_top5_2 = accuracy(test_outputs_2, labelts_2)
                        print("pred_top5_2", pred_top5_2)
                        test_5_epoch_2 += pred_top5_2
                    ...
                    test_acc_n = (test_correct / test_total).data.cpu().numpy()  #webvision
                    top_5 = (test_5_epoch / test_total).data.cpu().numpy()
                    #============================================================
                    test_acc_n_2 = (test_correct_2 / test_total_2).data.cpu().numpy()   #imagenet_2012
                    top_5_2 = (test_5_epoch_2 / test_total_2).data.cpu().numpy()
                save_csv(logs_filepath, experiment_name,
                         [epoch + 1, train_loss_n, train_acc_n,
                          noise_n, noise_pre_n, noise_zip_n,
                          jiu_true_n, pre_cs_n, pre_lab_cs_n,
                          "val_loss", "val_acc",
                          test_loss_n, test_acc_n, top_5,test_acc_n_2,top_5_2])
