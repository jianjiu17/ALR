...
class imagenet_dataset(Dataset):
    def __init__(self, root_dir, transform, num_class):
        ...
        self.gt_label = open(os.path.join(root_dir,'ILSVRC2012_devkit_t12/data/val.txt'))
        self.original_image_filepath = os.path.join(root_dir, 'ILSVRC2012_img_val/')
        #self.root = root_dir + 'ILSVRC2012_img_val/'
...
"""class imagenet_dataset(Dataset):
    def __init__(self, root_dir, transform, num_class):
        ...
        with open(self.root + 'ILSVRC2012_devkit_t12/data/val.txt') as f:
            lines = f.readlines()
            self.val_imgs = []
            self.val_labels = {}
            for line in lines:
    ...
    def __getitem__(self, index):
        ...
        image = Image.open(self.root + 'ILSVRC2012_img_val/' + img_path).convert('RGB')
        img = self.transform(image)
